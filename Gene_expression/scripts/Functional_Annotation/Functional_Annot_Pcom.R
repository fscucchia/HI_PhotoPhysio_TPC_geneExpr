...
#2 out of 29,874 UniProtKB AC/ID identifiers were successfully mapped to UniProtKB IDs
...
#86 out of 29,874 RefSeq Protein identifiers were successfully mapped to UniProtKB IDs 
...
#1771 out of 29,874 EMBL/GenBank/DDBJ CDS identifiers were successfully mapped to UniProtKB
...
#202 out of 29,874 Ensembl Genomes Protein identifiers were successfully mapped to UniProtKB IDs 
...
interpro <- read_tsv("/scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Functional_Annotation/Pcom/pcom.interpro.tsv", col_names = FALSE)
...
interpro_final <- interpro %>%
  ...
  ungroup()     ### 17303 genes with Go annotations
...
# Load InterProScan results
eggnog <- read_tsv("/work/pi_hputnam_uri_edu/HI_Genomes/Pcompressa/Porites_compressa_HIv1.genes.EggNog_results.txt", col_names = TRUE)
...
#retain only rows with GO terms
eggnog <- eggnog %>% filter(grepl("GO", go_ids))  ### 9988 genes with Go annotations
...
# Save as CSV
write_csv(final_GO_table_Pcom, "final_GO_table_Pcom.csv")
