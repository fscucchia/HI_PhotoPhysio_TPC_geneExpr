...
blast <- read_tsv("/scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Functional_Annotation/Pacu/Pacu.diamondBlastpNCBInr", col_names = FALSE)
...
#0 out of 25,486 UniProtKB AC/ID identifiers were successfully mapped to UniProtKB IDs
...
#37 out of 25,486 RefSeq Protein identifiers were successfully mapped to UniProtKB IDs 
...
#7183 out of 25,486 EMBL/GenBank/DDBJ CDS identifiers were successfully mapped to UniProtKB
...
#3198 out of 25,486 Ensembl Genomes Protein identifiers were successfully mapped to UniProtKB IDs 
...
#compile results
Uniprot_results2 <- bind_rows(uni2, uni3, uni4)
...
Uniprot_results_unique2 <- Uniprot_results_unique2[,c(1:4,8:11)]  #1061 Genes with GO terms 
colnames(Uniprot_results_unique2)[8] <- "protein_accession"

##### InterpoScan
# Load InterProScan results
interpro <- read_tsv("/scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Functional_Annotation/Pacu/macu.interpro.tsv", col_names = FALSE)
...
interpro_final <- interpro %>%
  ...
  ungroup()     ### 14540 genes with Go annotations
...
# Load InterProScan results
eggnog <- read_tsv("/work/pi_hputnam_uri_edu/HI_Genomes/PacutaV2/Pocillopora_acuta_HIv2.genes.EggNog_results.txt", col_names = TRUE)
...
#retain only rows with GO terms
eggnog <- eggnog %>% filter(grepl("GO", go_ids))  ### 9163 genes with Go annotations
...
# Save as CSV
write_csv(final_GO_table_Pacu, "final_GO_table_Pacu.csv")
