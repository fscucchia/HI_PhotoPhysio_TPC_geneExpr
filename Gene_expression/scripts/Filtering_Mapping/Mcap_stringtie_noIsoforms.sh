#!/bin/bash
#SBATCH --job-name=Mcap-stringtie
#SBATCH --nodes=1 --cpus-per-task=15
#SBATCH --mem=200G  # Requested Memory
#SBATCH -t 24:00:00
#SBATCH -o slurm-Mcap-stringtie_noIso.out  # %j = job ID
#SBATCH -e slurm-Mcap-stringtie_noIso.err  # %j = job ID
...
mkdir -p Stringtie_noIsoforms
  ...
  for i in ${array1[@]}; do
        stringtie -p 8 --rf -e -G /work/pi_hputnam_uri_edu/snRNA_analysis/references/Montipora_capitata_HIv3.gtf -A ${i}.gene_abund_noIsoforms.tab -o ${i}_noIsoforms.gtf ${i}
        mv ${i}_noIsoforms.gtf /scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Mapping_twopassMode_Mcap/Stringtie_noIsoforms
        mv ${i}.gene_abund_noIsoforms.tab /scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Mapping_twopassMode_Mcap/Stringtie_noIsoforms
