#!/bin/bash
#SBATCH --job-name=Pcom-mergeCompare_IsoformSwitch
#SBATCH --nodes=1 --cpus-per-task=15
#SBATCH --mem=200G  # Requested Memory
#SBATCH -t 24:00:00
#SBATCH -o slurm-Pcom-mergeCompare_IsoformSwitch.out  # %j = job ID
#SBATCH -e slurm-Pcom-mergeCompare_IsoformSwitch.err  # %j = job ID
...
#stringtie --merge -p 8 -G /work/pi_hputnam_uri_edu/snRNA_analysis/references/Porites_compressa_HIv_modified.gtf list_to_merge_IsoformSwitch.txt -o ./stringtie_merged_IsoformSwitch.gtf 

## Use the program gffcompare to compare the merged GTF files to the reference genome

gffcompare -r /work/pi_hputnam_uri_edu/snRNA_analysis/references/Porites_compressa_HIv_modified.gtf stringtie_merged_IsoformSwitch.gtf -o ./compared_IsoformSwitch 
