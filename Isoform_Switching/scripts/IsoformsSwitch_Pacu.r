...
setwd("/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu")

# Read GTF and fix strand information
gtf <- import("/scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Mapping_twopassMode_Pacu/stringtie_merged_IsoformSwitch.gtf")
...
gffread -w transcripts.fa -g /work/pi_hputnam_uri_edu/HI_Genomes/PacutaV2/Pocillopora_acuta_HIv2.assembly.fasta /scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Mapping_twopassMode_Pacu/stringtie_merged_IsoformSwitch.gtf

# Create sampleTable for IsoformSwitchAnalyzeR
sampleTable <- data.frame(
    sampleID = c("B10","G10", "H10",    # Control samples
                 "B7", "F7", "G7", "H7",        # 35°C samples
...
switchList <- importRdata(
    ...
    isoformExonAnnoation = "/scratch/workspace/federica_scucchia_uri_edu-hawaii/20250424_ENCORE_HawaiiTPC_Federica/output/Mapping_twopassMode_Pacu/stringtie_merged_IsoformSwitch.gtf",
    showProgress = TRUE
)
...
# 1 control vs t30                289 - 482
# 2 control vs t35              1447 - 2412
# 3     t30 vs t35              1251 - 2085
...
#The filtering removed 45164 ( 39.67% of ) transcripts. There is now 68682 isoforms left
...
#Isoform switch analysis was performed for 46375 gene comparisons (100%).
...
#[1] "Total number of significant isoform switches: 6125"

# Save the analyzed switchList for further analysis
save(switchListAnalyzed, file = "switchListAnalyzed_Pacu.RData")
...
switchListAnalyzed <- addORFfromGTF(
    switchAnalyzeRlist = switchListAnalyzed,
    pathToGTF = "/work/pi_hputnam_uri_edu/snRNA_analysis/references/Pocillopora_acuta_HIv2_modified.gtf"
)
# Step 1 of 2: importing GTF (this may take a while)...
# Step 2 of 2: Adding ORF...
#     Added ORF info (incl info about isoforms annotated as not having an ORF) to 3393 isoforms.
#         This correspond to 11.4%% of isoforms in the switchAnalyzeRlist.

#Only 11.4% of isoforms got ORF info - This is normal for StringTie because most of the isoforms are novel transcripts discovered by StringTie
# 3393 isoforms got ORF annotations from known genes
...
#29234 putative ORFs were identified, analyzed and added.
...
switchListAnalyzed <- extractSequence(
    switchAnalyzeRlist = switchListAnalyzed,
    pathToOutput = "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu",
    writeToFile = TRUE
)
...
switchListAnalyzed <- extractSequence(
    switchAnalyzeRlist = switchListAnalyzed,
    pathToOutput = "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files",
    writeToFile = TRUE,
...
nt_seqs <- readDNAStringSet("/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/isoformSwitchAnalyzeR_isoform_nt.fasta")
...
for(i in 1:n_chunks) {
  ...
  writeXStringSet(chunk_seqs, 
                  filepath = paste0("/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/switchList_nt_part_", i, ".fasta"))
}

# Get the total number of genes in Pacu genome
# Option 1: From reference GTF file
ref_gtf <- import("/work/pi_hputnam_uri_edu/snRNA_analysis/references/Pocillopora_acuta_HIv2_modified.gtf")
total_genes_genome <- length(unique(ref_gtf$gene_id))

genes_with_switching <- 4246  # From Combined nrGenes result
...
#[1]  "Genes with temperature-responsive isoform switching: 4246"
print(paste("Total genes in genome:", total_genes_genome))
#[1]  "Total genes in genome: 33730"
...
#[1] "Switching isoforms out of total analyzed: 7.6%"
...
cpc2_files <- c(
  "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/result_cpc2_nt_part1.txt",
  "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/result_cpc2_nt_part2.txt"
)
...
write.table(cpc2_combined, 
           "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/cpc2_combined_results.txt",
...
base_path <- "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/split_files/"
...
#Added coding potential to 24518 (82.37%) transcripts

# Add Pfam analysis (protein domains)
switchListAnalyzed <- analyzePFAM(
    switchAnalyzeRlist = switchListAnalyzed,
    pathToPFAMresultFile = paste0(base_path, "Pacu_pfam_results_complete.txt"),
    showProgress = TRUE
)
#Added domain information to 17480 (58.73%) transcripts
...
# 16530  5550  1649   536   159    69    13     8     2     1     1 
...
print(summary_all)
#       Comparison nrIsoforms nrSwitches nrGenes
# 1 control vs t30        495        616     416
# 2 control vs t35       4440       5707    2953
# 3     t30 vs t35       3940       5113    2756
# 4       Combined       6754       9157    4246
...
# control vs t30: 611/616 switches (99.2%) in 394/416 genes (94.7%) have functional consequences
# control vs t35: 5684/5707 switches (99.6%) in 2870/2953 genes (97.2%) have functional consequences
# t30 vs t35: 5086/5113 switches (99.5%) in 2664/2756 genes (96.7%) have functional consequences
# Combined: 9113/9157 switches (99.5%) in 4131/4246 genes (97.3%) have functional consequences
...
for(comp_name in names(top_genes_by_comparison)) {
    ...
    write.csv(top_genes_by_comparison[[comp_name]], 
              paste0("/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/top10_genes_effect_size_", file_name, ".csv"),
              row.names = FALSE)
}
...
for(comp_name in names(top_isoforms_by_comparison)) {
    ...
    write.csv(top_isoforms_by_comparison[[comp_name]], 
              paste0("/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/top10_isoforms_effect_size_", file_name, ".csv"),
              row.names = FALSE)
}
...
plot_output_dir <- "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/isoforms_plots/"
...
#save as 4x16 landscape 	splicing_enrichment_allisoforms.pdf
...
# 1         IR              7988        11608                32.6               1.45
# 2         A5              9973        13700                40.7               1.37
# 3         A3             11498        17734                46.9               1.54
# 4         ES             10110        13685                41.2               1.35
# 5        MES              6695         8193                27.3               1.22
# 6       ATSS             15102        15102                61.6               1.00
# 7       ATTS             13677        13677                55.8               1.00

write.csv(global_as_summary, "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/global_splicing_events.csv", row.names = FALSE)
...
#Total unique switching genes:  4248  ->  3653 
...
#Percentage of switching genes in WGCNA:  86 %
...
all_hub_genes_control <- unique(c(blue_hub_genes_control_unique, turquoise_hub_genes_control_unique))
all_hub_genes_30 <- unique(c(blue_hub_genes_30_unique, turquoise_hub_genes_30_unique))
all_hub_genes_35 <- unique(c(blue_hub_genes_35_unique, turquoise_hub_genes_35_unique))
...
write.csv(overlap_results_unique, "unique_hub_switching_overlap_summary_Pacu.csv", row.names = FALSE)
...
print(p7_temp_ordered) #save as landscape 6x4 	switch_isof_hub_genes_Pacu.pdf
...
#Total genes with isoforms:  4056
...
cat(sprintf("Control hub genes switching: %d/%d (%s%%)\n", 
           ...
           #Control hub genes switching: 262/2037 (12.9%)
cat(sprintf("Non-control hub genes switching: %d/%d (%s%%)\n", 
           ...
           #Non-control hub genes switching: 3236/23386 (13.8%)
...
write.csv(results_summary, "/scratch3/workspace/federica_scucchia_uri_edu-altSplice/20250424_ENCORE_HawaiiTPC_Federica/output/Rstudio/Isoform_switch/Pacu/combined_35C_fisher_results.csv", row.names = FALSE)